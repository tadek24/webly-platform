  ...
  return (
    <div className="marketing-shell">
      <header className="site-header">
        ...
        <Link className="header-panel-link" href="/panel">Panel klienta <span>↗</span></Link>
        <details className="mobile-menu">
          <summary>MENU +</summary>
          <div>{nav.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}<Link href="/panel">Panel klienta ↗</Link></div>
        </details>
      </header>
      <main>{children}</main>
      <footer className="site-footer">
        ...
        <div><small>PRODUKT</small><Link href="/panel">Panel klienta</Link><Link href="/oferta#faq">FAQ</Link><Link href="/kontakt">Rozpocznij projekt</Link></div>
